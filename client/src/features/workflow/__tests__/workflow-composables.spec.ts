...
import type { BookWorkflowPreference, BookWorkflowStatus, CreateWorkflowRequest, WorkflowDetail } from '@bookorbit/types'
...
vi.mock('../api/workflow', () => ({
  ...
  getBookWorkflowPreference: vi.fn<(bookId: number) => Promise<BookWorkflowPreference>>(),
  runBookWorkflow: vi.fn<(bookId: number, workflowId: number) => Promise<void>>(),
  setBookWorkflowPreference: vi.fn<(bookId: number, workflowId: number | null) => Promise<void>>(),
}))
...
describe('useBookWorkflows', () => {
  ...
  it('refreshes statuses and preference concurrently', async () => {
    vi.mocked(apiModule.getBookWorkflowStatuses).mockResolvedValueOnce([sampleStatus])
    vi.mocked(apiModule.getBookWorkflowPreference).mockResolvedValueOnce({ workflowId: 1 })
    const { statuses, preferenceWorkflowId, loading, refresh } = useBookWorkflows()

    await refresh(100)

    expect(loading.value).toBe(false)
    expect(statuses.value).toEqual([sampleStatus])
    expect(preferenceWorkflowId.value).toBe(1)
    expect(apiModule.getBookWorkflowStatuses).toHaveBeenCalledWith(100)
    expect(apiModule.getBookWorkflowPreference).toHaveBeenCalledWith(100)
  })
  ...
  it('optimistically flips status to running during run() and refreshes afterward', async () => {
    ...
    vi.mocked(apiModule.getBookWorkflowPreference).mockResolvedValue({ workflowId: 1 })
    vi.mocked(apiModule.runBookWorkflow).mockImplementation(async () => {
      // While running, status should be optimistically flipped
  ...
  it('updates preferenceWorkflowId upon setPreference', async () => {
    vi.mocked(apiModule.setBookWorkflowPreference).mockResolvedValueOnce()
    const { preferenceWorkflowId, setPreference } = useBookWorkflows()

    await setPreference(100, 5)

    expect(apiModule.setBookWorkflowPreference).toHaveBeenCalledWith(100, 5)
    expect(preferenceWorkflowId.value).toBe(5)

    await setPreference(100, null)
    expect(apiModule.setBookWorkflowPreference).toHaveBeenCalledWith(100, null)
    expect(preferenceWorkflowId.value).toBeNull()
  })
})
