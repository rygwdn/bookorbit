...
const { t } = useI18n()
const shellModes = ref<Record<number, boolean>>({})
const shellValues = ref<Record<number, string>>({})
const shellErrors = ref<Record<number, string>>({})
...
function toggleCommandMode(index: number) {
  const enabled = shellModes.value[index] === true
  if (enabled) {
    try {
      const parsed = parseShellCommand(shellValues.value[index] ?? '')
      updateStep(index, parsed)
      shellErrors.value = { ...shellErrors.value, [index]: '' }
    } catch (error) {
      shellErrors.value = { ...shellErrors.value, [index]: error instanceof Error ? error.message : 'Invalid command' }
      return
    }
  } else {
    shellValues.value = { ...shellValues.value, [index]: formatShellCommand(steps.value[index]!.command, steps.value[index]!.args) }
  }
  shellModes.value = { ...shellModes.value, [index]: !enabled }
}

function setShellCommand(index: number, value: string) {
  shellValues.value = { ...shellValues.value, [index]: value }
  if (shellErrors.value[index]) shellErrors.value = { ...shellErrors.value, [index]: '' }
}
      ...
      <div class="flex items-start gap-2">
        ...
        <div class="min-w-0 flex-1 space-y-2.5">
          ...
          <div>
            ...
            <button type="button" class="mt-1 text-xs text-muted-foreground hover:text-foreground" @click="toggleCommandMode(index)">
              {{ shellModes[index] ? t('settings.admin.workflows.stepEditor.structuredMode') : t('settings.admin.workflows.stepEditor.shellMode') }}
            </button>
            <textarea
              v-if="shellModes[index]"
              :value="shellValues[index]"
              class="input-field mt-1 min-h-16 w-full font-mono text-xs"
              :placeholder="t('settings.admin.workflows.stepEditor.shellPlaceholder')"
              @input="setShellCommand(index, ($event.target as HTMLTextAreaElement).value)"
            />
            <p v-if="shellErrors[index]" class="mt-1 text-xs text-destructive">{{ shellErrors[index] }}</p>
