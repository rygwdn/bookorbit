...
const { statuses, preferenceWorkflowId, loading, refresh, run, setPreference } = useBookWorkflows()
...
const successfulWorkflows = computed(() => workflows.value.filter((w) => statusFor(w.id)?.status === 'success'))
...
async function handlePreferenceChange(event: Event) {
  const raw = (event.target as HTMLSelectElement).value
  const workflowId = raw === '' ? null : Number(raw)
  preferenceError.value = null
  try {
    await setPreference(props.book.id, workflowId)
  } catch (e) {
    preferenceError.value = e instanceof Error ? e.message : t('book.detail.workflows.preferenceError')
  }
}
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="successfulWorkflows.length > 0" class="flex flex-col gap-1.5 rounded-md border border-border bg-muted/30 px-4 py-3">
      <label for="workflow-preference" class="text-sm font-medium text-foreground">{{ t('book.detail.workflows.preference.label') }}</label>
      <select
        id="workflow-preference"
        :value="preferenceWorkflowId ?? ''"
        class="h-9 max-w-sm px-2 rounded-md border border-border bg-background text-sm"
        @change="handlePreferenceChange"
      >
        <option value="">{{ t('book.detail.workflows.preference.none') }}</option>
        <option v-for="wf in successfulWorkflows" :key="wf.id" :value="wf.id">{{ wf.name }}</option>
      </select>
      <p v-if="preferenceError" class="text-xs text-destructive">{{ preferenceError }}</p>
    </div>
