...
import { AuditAction, AuditResource, Permission, type BookWorkflowPreference, type BookWorkflowStatus } from '@bookorbit/types';
...
import { SetWorkflowPreferenceDto } from './dto/set-workflow-preference.dto';
...
export class WorkflowRunController {
  ...
  @Get('preference')
  async getPreference(@Param('bookId', ParseIntPipe) bookId: number, @CurrentUser() user: RequestUser): Promise<BookWorkflowPreference> {
    await this.bookService.verifyBookAccess(bookId, user);
    return this.workflowRunnerService.getPreference(user.id, bookId);
  }
  ...
  @Put('preference')
  @Auditable({
    action: AuditAction.WorkflowPreferenceSet,
    resource: AuditResource.Workflow,
    getResourceId: (req) => parseInt(req.params['bookId'] ?? '', 10) || undefined,
    description: (req) => `Set workflow preference for book #${req.params['bookId']}`,
  })
  async setPreference(
    @Param('bookId', ParseIntPipe) bookId: number,
    @Body() dto: SetWorkflowPreferenceDto,
    @CurrentUser() user: RequestUser,
  ): Promise<void> {
    await this.bookService.verifyBookAccess(bookId, user);
    await this.workflowRunnerService.setPreference(user.id, bookId, dto.workflowId);
  }
}
