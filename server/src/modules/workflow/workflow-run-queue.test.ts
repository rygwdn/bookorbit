...
type ResolverFactory = <T>() => { promise: Promise<T>; resolve: (value: T | PromiseLike<T>) => void };
const promiseConstructor = Promise as unknown as { withResolvers: ResolverFactory };
const withResolvers: ResolverFactory = promiseConstructor.withResolvers.bind(Promise);

describe('WorkflowRunQueue', () => {
  it('runs queued work within the configured concurrency', async () => {
    let active = 0;
    let maxActive = 0;
    const { promise: firstDone, resolve: releaseFirst } = withResolvers<void>();
    const { promise: firstStart, resolve: firstStarted } = withResolvers<void>();
    const { promise: secondStart, resolve: secondStarted } = withResolvers<void>();
  ...
  it('processes queued items in lifo order when configured', async () => {
    const handled: number[] = [];
    const { promise: firstBlocked, resolve: releaseFirst } = withResolvers<void>();
    const { promise: firstStarted, resolve: markFirstStarted } = withResolvers<void>();
